...
将来、自作 Web ページや LLM からの呼び出し、状態変化の購読、キャッシュなどが要件として出てくる。これらは **casa に足さない**。casa の上にもう一層（仮称 `casad`）を作って吸収する。

```
Web ページ / LLM / その他クライアント
       │
       ▼
   上層（casad: 常駐・状態を持つ。別リポジトリ）
       │
...
### `casad` 側が担う想定の責務（casa の責務ではない）
- HTTP / WebSocket / MCP サーバ
- 値のキャッシュとフレッシュネス管理
- ECHONET の INF 通知の購読（`enl watch` 経由を想定）
- LLM からの Function Calling 受け口
- 認証・認可・レート制限
...
`casad` の設計・実装は本リポジトリのスコープ外。別リポジトリで進める。
...
- **状態変化の監視（INF 通知の待受）**。`casad` の責務。casa は購読を持たない。
- **デーモン・常駐モード**。casa 本体には実装しない。常駐が必要なら `casad` を別リポジトリで作る。
...
- デーモン化・常駐・内部スケジューラを足さない（必要なら別リポジトリで `casad` を作る）。
